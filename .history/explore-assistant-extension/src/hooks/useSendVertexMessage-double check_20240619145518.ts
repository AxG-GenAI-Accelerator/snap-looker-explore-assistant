...
import { addMessage } from '../slices/assistantSlice' 
...
const useSendVertexMessage = () => {
  ...
  console.log("messageThread const: ", messageThread)
...
${exploreRefinementExamples
  ...
  const generateExploreUrl = useCallback(
    ...
    `
      ...
      console.log('generateExploreURL parameters:', parameters)
      ...
      const metadataParams = [...dimensions.map(d => d.name), ...measures.map(m => m.name)];
      console.log('metadataParams: ', metadataParams);

      const isValid = fieldsFromUrl.every(field => metadataParams.includes(field));
      ...
      if (!isValid) {
        ...
        const metadataParamsString = metadataParams.join(', ');
        ...
        dispatch(addMessage({
          ...
          createdAt: Date.now(),
          type: 'text',
          intent: 'exploreRefinement',
        }));

        await sendMessageWithThread(errorPrompt, newExploreUrl, metadataParamsString, true);
  ...
  const sendMessageWithThread = useCallback(
    async (prompt: string, originalUrl: string, metadataParams: string, isError: boolean = false) => {
      console.log("messageThread sendMessageWithThread: ", messageThread)
      const messageHistoryContents = messageThread
        .slice(-20)
        .map((message) => {
          console.log('Thread Message:', message); // Add logging to check each message
          if ('exploreUrl' in message) {
            console.log('exploreURL in message: ', message.exploreUrl)
             return `${message.actor}: ${message.exploreUrl}`; // Optionally filter out certain types of messages if not relevant
          }
          return `${message.actor}: ${message.message}`;
        })
        .join('\n');
        console.log('Filtered Messages: ', messageHistoryContents); // Log the filtered messages
