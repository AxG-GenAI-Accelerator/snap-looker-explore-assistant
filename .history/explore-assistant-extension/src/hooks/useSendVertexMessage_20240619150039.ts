...
${exploreRefinementExamples
  ...
  const generateExploreUrl = useCallback(
    async (prompt: string) => {
      //UPDATE CONTEXT BASED ON USE CASE
      const contents = `
        Context
        ----------
        ...
        - All URLs should mention the lookertestv8 explore followed by the desired dimension, for example, lookertestv8.footfall. It will always be lookertestv8.
        ...
        - If there is a prompt asking for partner distribution show it as a bar graph with the partner_grouping dimension first and only one other dimension which the user specifies, for examples sales or growth potential along with others
        LookML Metadata 
        ----------
