...
export const useBigQueryExamples = () => {
  ...
  const getExamplePrompts = async () => {
    ...
    return runExampleQuery(sql).then((response) => {
      ...
      console.log('useBQ setExploreGenerationExamples : ',      dispatch(setExploreGenerationExamples(generationExamples)))

      console.log('useBQ setExploreGenerationExamples length: ',      dispatch(setExploreGenerationExamples(generationExamples)).length)
    })
  }
  ...
  `
    return runExampleQuery(sql).then((response) => {
      ...
      console.log('use BQ setExploreRefinementExamples: ',setExploreRefinementExamples(refinementExamples))
      console.log('use BQ setExploreRefinementExamples length: ',setExploreRefinementExamples(refinementExamples).length)
